    ...
    <>
      {await Promise.all(
        rows.map(async (row) => {
          ...
          const media = row.media as Record<string, { url?: string; translations?: Array<{ altText?: string }> }> | undefined;
          const mediaItem = typeof content.mediaId === 'string' ? media?.[content.mediaId] : undefined;
          ...
          if (type === 'hero') {
            ...
            return <section className="hero" key={String(row.id)}><div className="container hero-grid"><div>{Boolean(content.eyebrow) && <div className="eyebrow">{text(content.eyebrow)}</div>}<h1>{text(content.title)}</h1>{Boolean(content.body) && <p>{text(content.body)}</p>}<div className="hero-actions">{Boolean(primary?.label) && Boolean(primary?.href) && <Link className="gv-button" href={text(primary?.href)}>{text(primary?.label)}</Link>}{Boolean(secondary?.label) && Boolean(secondary?.href) && <Link className="text-link" href={text(secondary?.href)}>{text(secondary?.label)}</Link>}</div></div>{mediaItem?.url ? <figure>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={mediaItem.url} alt={mediaItem.translations?.[0]?.altText ?? ''} /></figure> : <div className="portal" aria-hidden="true" />}</div></section>;
          }

          if (type === 'text_image') {
            const image = mediaItem?.url ? <figure>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={mediaItem.url} alt={mediaItem.translations?.[0]?.altText ?? ''} /></figure> : null;
            return <section className="section section--surface" key={String(row.id)}><div className="container text-image">{content.mediaPosition === 'start' && image}<div className="rich-text">{Boolean(content.eyebrow) && <span className="eyebrow">{text(content.eyebrow)}</span>}{Boolean(content.title) && <h2>{text(content.title)}</h2>}<p>{text(content.body)}</p></div>{content.mediaPosition !== 'start' && image}</div></section>;
          }

          if (type === 'process' || type === 'timeline') {
            return <section className="section" key={String(row.id)}><div className="container">{Boolean(content.title) && <h2>{text(content.title)}</h2>}<ol className={type === 'timeline' ? 'timeline' : 'process-grid'}>{list(content.steps).map((rawStep, index) => { const step = rawStep as Record<string, unknown>; return <li key={index}><span className="eyebrow">{text(step.marker, String(index + 1).padStart(2, '0'))}</span><h3>{text(step.title)}</h3><p>{text(step.body)}</p></li>; })}</ol></div></section>;
          }
          ...
          if (type === 'logo_cloud' || type === 'ecosystem') {
            const collections = row.collections as Record<string, unknown[]> | undefined;
            const resources = type === 'logo_cloud' ? ['clients', 'partners'] : ['brands', 'products'];
            const items = resources.flatMap((resource) => (collections?.[resource] ?? []) as Record<string, unknown>[]);
            return <section className="section section--surface" key={String(row.id)}><div className="container">{Boolean(content.title) && <h2>{text(content.title)}</h2>}<div className="logo-cloud">{items.map((item) => { const itemTr = translation(item); const itemMedia = item.media as Record<string, { url?: string; translations?: Array<{ altText?: string }> }> | undefined; const logoId = text(item.logoMediaId); const logo = logoId ? itemMedia?.[logoId] : undefined; return <div className="logo-card" key={String(item.id)}>{logo?.url && <>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={logo.url} alt={logo.translations?.[0]?.altText ?? text(itemTr.name ?? itemTr.title)} /></>}<strong>{text(itemTr.name ?? itemTr.title)}</strong></div>; })}</div></div></section>;
          }

          if (type === 'form') {
            const formType = content.formType === 'assessment' ? 'market-entry-assessment' : content.formType === 'consultation' ? 'consultation' : 'contact';
            return <section className="section" key={String(row.id)}><div className="container">{Boolean(content.title) && <h2>{text(content.title)}</h2>}<LeadForm kind={formType} locale={locale} /></div></section>;
          }
          ...
          return (
            <section className="section section--surface" key={String(row.id)}>
              <div className="container rich-text">
                ...
                {Boolean(content.title) && <h2>{text(content.title)}</h2>}
                {Boolean(content.body) && <p>{text(content.body)}</p>}
                {Boolean(content.blocks) && <RichBlocks blocks={content.blocks} media={media ?? {}} />}
              </div>
            </section>
          );
        }),
  ...
  return (
    <>
      {list(blocks).map((raw, index) => {
        ...
        if (type === 'quote') return <blockquote key={index}>{text(block.text)}{Boolean(block.attribution) && <footer>{text(block.attribution)}</footer>}</blockquote>;
        if (type === 'list')
...
export function ContentItems({ items }: { items: unknown }) {
  return <ul>{list(items).map((raw, index) => {
    ...
    const item = raw as Record<string, unknown>;
    return <li key={index}>{Boolean(item.title) && <strong>{text(item.title)} </strong>}{text(item.body ?? item.label ?? item.value)}{text(item.suffix)}</li>;
  })}</ul>;
}
