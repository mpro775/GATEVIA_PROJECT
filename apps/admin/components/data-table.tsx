...
export function DataTable({ title, resource, basePath, kind = 'content', source }: Props) {
  ...
  return (
    <>
      ...
      {error ? (
        <ErrorState title="Unable to load records" description="Check your connection or access permissions." />
      ) : !state ? (
        <div className="panel">
          <Skeleton />
          <br />
        ...
        <EmptyState title="No records yet" description="Create the first approved record when content is ready." />
      ) : (
        <>
