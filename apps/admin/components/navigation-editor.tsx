...
import type { Language as ApiLanguage } from '@gatevia/api-client';
  ...
  return value.map((raw) => {
    ...
    const translations = Array.isArray(item.translations)
      ? Object.fromEntries(
          item.translations
            .filter(
              (row): row is { locale: string; label: string } =>
                Boolean(row) &&
                typeof row === 'object' &&
...
export function NavigationEditor() {
  ...
  useEffect(() => {
    void api<NavMenu[]>('/admin/navigation?pageSize=50').then((rows) =>
      setMenus((rows || []).map(normalizeMenu)),
    );
    ...
    const fetchEntities = async () => {
      ...
      await Promise.all(
        types.map(async (t) => {
          try {
            const res = await api<
              { id: string; translations?: { title?: string; name?: string }[] }[]
            >(`/admin/${t}?pageSize=100`);
            map[t] = (res || []).map((r) => ({
              id: r.id,
              label: r.translations?.[0]?.title || r.translations?.[0]?.name || r.id,
            }));
          } catch {
            map[t] = [];
          }
  ...
  async function transition(action: 'publish' | 'unpublish' | 'archive') {
    ...
    try {
      ...
      if (!saved) return;
      const updated = await api<NavMenu>(`/admin/navigation/${selected.id}/${action}`, {
        method: 'POST',
      });
