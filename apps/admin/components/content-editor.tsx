...
import { cmsDefinitions, fieldSchema, sectionSchemas, type CmsField, type SectionType } from '@gatevia/contracts';
...
const SEO_FIELDS = new Set(['seoTitle', 'seoDescription', 'ogTitle', 'ogDescription', 'ogMediaId', 'canonicalUrl', 'robotsIndex']);
...
const SECTION_FIELDS: Record<SectionType, SectionField[]> = {
  hero: [...BASE_SECTION_FIELDS, { key: 'body', label: 'Body', kind: 'long' }, { key: 'primaryCta', label: 'Primary CTA { label, href }', kind: 'json' }, { key: 'secondaryCta', label: 'Secondary CTA { label, href }', kind: 'json' }, { key: 'mediaId', label: 'Hero media', kind: 'media' }],
  rich_text: [{ key: 'blocks', label: 'Structured content blocks', kind: 'json' }],
  text_image: [...BASE_SECTION_FIELDS, { key: 'body', label: 'Body', kind: 'long' }, { key: 'mediaId', label: 'Media', kind: 'media' }, { key: 'mediaPosition', label: 'Media position', kind: 'select', options: ['start', 'end'] }],
  stats: [...BASE_SECTION_FIELDS, { key: 'items', label: 'Stats items', kind: 'json' }],
  services_grid: [...BASE_SECTION_FIELDS, { key: 'serviceIds', label: 'Services', kind: 'relation', resource: 'services' }, { key: 'featuredOnly', label: 'Featured only', kind: 'boolean' }],
  industries_grid: [...BASE_SECTION_FIELDS, { key: 'industryIds', label: 'Industries', kind: 'relation', resource: 'industries' }, { key: 'featuredOnly', label: 'Featured only', kind: 'boolean' }],
  ...
  testimonials: [...BASE_SECTION_FIELDS, { key: 'testimonialIds', label: 'Testimonials', kind: 'relation', resource: 'testimonials' }],
  case_studies: [...BASE_SECTION_FIELDS, { key: 'caseStudyIds', label: 'Case studies', kind: 'relation', resource: 'case-studies' }],
  logo_cloud: [...BASE_SECTION_FIELDS, { key: 'clientIds', label: 'Clients', kind: 'relation', resource: 'clients' }, { key: 'partnerIds', label: 'Partners', kind: 'relation', resource: 'partners' }],
  faq: [...BASE_SECTION_FIELDS, { key: 'faqIds', label: 'FAQs', kind: 'relation', resource: 'faqs' }],
  cta: [...BASE_SECTION_FIELDS, { key: 'body', label: 'Body', kind: 'long' }, { key: 'primaryCta', label: 'Primary CTA { label, href }', kind: 'json' }],
  insights: [...BASE_SECTION_FIELDS, { key: 'insightIds', label: 'Insights', kind: 'relation', resource: 'insights' }],
  ecosystem: [...BASE_SECTION_FIELDS, { key: 'brandIds', label: 'Brands', kind: 'relation', resource: 'brands' }, { key: 'productIds', label: 'Products', kind: 'relation', resource: 'products' }],
  form: [...BASE_SECTION_FIELDS, { key: 'formType', label: 'Form type', kind: 'select', options: ['contact', 'consultation', 'assessment'] }],
};

function humanize(value: string) {
  return value.replace(/([a-z])([A-Z])/g, '$1 $2').replaceAll('_', ' ').replaceAll('-', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase());
}
function normalizeTranslations(value: unknown): TranslationMap {
  if (Array.isArray(value)) return Object.fromEntries(value.filter((row): row is Record<string, unknown> => Boolean(row) && typeof row === 'object').map((row) => [String(row.locale), row]));
...
function normalizeSections(value: unknown): PageSection[] {
  ...
  return value.map((raw) => {
    ...
    return {
      ...
      translations: Object.fromEntries(Object.entries(translations).map(([locale, tr]) => [locale, { content: tr.content as Record<string, unknown> ?? {} }])),
    };
  });
}
async function loadOptions(resource: string): Promise<RelOption[]> {
  try {
    const rows = await api<Array<{ id: string; displayName?: string; translations?: Array<Record<string, unknown>> }>>(`/admin/${resource}?pageSize=100`);
    return rows.map((row) => {
      const tr = row.translations?.[0] ?? {};
      return { id: row.id, label: String(tr.title ?? tr.name ?? tr.question ?? row.displayName ?? row.id) };
    });
  } catch { return []; }
}

function StructuredField({ label, value, expected, onChange }: { label: string; value: unknown; expected: 'array' | 'object-or-array'; onChange: (value: unknown) => void }) {
  const formatted = useMemo(() => JSON.stringify(value ?? (expected === 'array' ? [] : {}), null, 2), [value, expected]);
  ...
  function commit(next: string) {
    ...
    try {
      ...
      if (expected === 'array' && !Array.isArray(parsed)) throw new Error('Value must be a JSON array.');
      if (expected === 'object-or-array' && (!parsed || typeof parsed !== 'object')) throw new Error('Value must be valid structured JSON.');
      setError(''); onChange(parsed);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Invalid JSON.'); }
  }
  return <Field label={label}><Textarea dir="ltr" value={draft} onChange={(event) => commit(event.target.value)} rows={8} />{error && <span className="cell-meta" role="alert">{error}</span>}</Field>;
}

function RelationSelect({ label, options, selectedIds, onChange }: { label: string; options: RelOption[]; selectedIds: string[]; onChange: (ids: string[]) => void }) {
  return <Field label={label}><div className="relation-chips">{options.length === 0 && <span className="cell-meta">No options available.</span>}{options.map((option) => {
    const selected = selectedIds.includes(option.id);
    return <label key={option.id} className="relation-chip" style={{ cursor: 'pointer' }}><input type="checkbox" checked={selected} onChange={() => onChange(selected ? selectedIds.filter((id) => id !== option.id) : [...selectedIds, option.id])} style={{ display: 'none' }} /><Badge tone={selected ? 'success' : 'neutral'}>{option.label}</Badge></label>;
  })}</div></Field>;
}

function ContractField({ name, field, value, onChange }: { name: string; field: CmsField; value: unknown; onChange: (value: unknown) => void }) {
  const label = `${humanize(name)}${field.required ? ' *' : ''}`;
  if (field.kind === 'media') return <Field label={label}><MediaPicker value={typeof value === 'string' ? value : ''} onSelect={(mediaId) => onChange(mediaId || null)} /></Field>;
  if (field.kind === 'boolean') return <label><input type="checkbox" checked={Boolean(value)} onChange={(event) => onChange(event.target.checked)} />{' '}{label}</label>;
  if (field.kind === 'items' || field.kind === 'blocks') return <StructuredField label={label} value={value ?? []} expected="array" onChange={onChange} />;
  if (field.kind === 'select') return <Field label={label}><select className="gv-input" value={String(value ?? '')} onChange={(event) => onChange(event.target.value)}><option value="">— Select —</option>{field.options?.map((option) => <option key={option} value={option}>{humanize(option)}</option>)}</select></Field>;
  if (field.kind === 'long') return <Field label={label}><Textarea value={String(value ?? '')} onChange={(event) => onChange(event.target.value)} /></Field>;
  return <Field label={label}><Input dir={field.kind === 'url' || field.kind === 'slug' || field.kind === 'date' ? 'ltr' : undefined} type={field.kind === 'number' ? 'number' : 'text'} value={String(value ?? '')} onChange={(event) => onChange(field.kind === 'number' ? Number(event.target.value) : event.target.value)} required={field.required} placeholder={field.kind === 'date' ? '2026-09-08T00:00:00.000Z' : undefined} /></Field>;
}

function SectionsEditor({ sections, locale, options, onChange }: { sections: PageSection[]; locale: string; options: Record<string, RelOption[]>; onChange: (next: PageSection[]) => void }) {
  function mutate(index: number, mutation: (section: PageSection) => PageSection) { onChange(sections.map((section, current) => current === index ? mutation(section) : section)); }
  function updateContent(index: number, key: string, value: unknown) {
    mutate(index, (section) => {
      ...
      return { ...section, translations: { ...section.translations, [locale]: { content: { ...current, [key]: value } } } };
    });
  }
  ...
  function changeType(index: number, sectionType: SectionType) {
    mutate(index, (section) => ({ ...section, sectionType, translations: Object.fromEntries(Object.keys(section.translations).map((translationLocale) => [translationLocale, { content: {} }])) }));
  }
  return <div><div className="sections-editor">{sections.length === 0 && <p className="cell-meta">No sections yet.</p>}{sections.map((section, index) => {
    const content = section.translations[locale]?.content ?? {};
    return <div key={section.id ?? index} className="section-row" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}><span className="section-type">{humanize(section.sectionType)}</span><Badge tone={section.isVisible ? 'success' : 'neutral'}>{section.isVisible ? 'Visible' : 'Hidden'}</Badge><div className="section-actions"><button type="button" className="text-link" onClick={() => move(index, -1)} disabled={index === 0}>↑</button><button type="button" className="text-link" onClick={() => move(index, 1)} disabled={index === sections.length - 1}>↓</button><button type="button" className="text-link" onClick={() => mutate(index, (row) => ({ ...row, _expanded: !row._expanded }))}>{section._expanded ? 'Collapse' : 'Edit'}</button><button type="button" className="text-link" style={{ color: 'var(--color-danger)' }} onClick={() => onChange(sections.filter((_, current) => current !== index))}>Delete</button></div></div>
      {section._expanded && <div className="section-fields">
        <Field label="Section type"><select className="gv-input" value={section.sectionType} onChange={(event) => changeType(index, event.target.value as SectionType)}>{SECTION_TYPES.map((type) => <option key={type} value={type}>{humanize(type)}</option>)}</select></Field>
        <Field label="Theme"><select className="gv-input" value={section.settings.theme ?? 'default'} onChange={(event) => mutate(index, (row) => ({ ...row, settings: { ...row.settings, theme: event.target.value as 'default' | 'inverse' } }))}><option value="default">Default</option><option value="inverse">Inverse</option></select></Field>
        <label><input type="checkbox" checked={section.isVisible} onChange={(event) => mutate(index, (row) => ({ ...row, isVisible: event.target.checked }))} /> Visible</label>
        {(SECTION_FIELDS[section.sectionType] ?? []).map((field) => {
          if (field.kind === 'relation') return <RelationSelect key={field.key} label={field.label} options={options[field.resource] ?? []} selectedIds={Array.isArray(content[field.key]) ? content[field.key] as string[] : []} onChange={(value) => updateContent(index, field.key, value)} />;
          if (field.kind === 'media') return <Field key={field.key} label={field.label}><MediaPicker value={String(content[field.key] ?? '')} onSelect={(value) => updateContent(index, field.key, value)} /></Field>;
          if (field.kind === 'boolean') return <label key={field.key}><input type="checkbox" checked={Boolean(content[field.key])} onChange={(event) => updateContent(index, field.key, event.target.checked)} /> {field.label}</label>;
          if (field.kind === 'select') return <Field key={field.key} label={field.label}><select className="gv-input" value={String(content[field.key] ?? '')} onChange={(event) => updateContent(index, field.key, event.target.value)}><option value="">— Select —</option>{field.options.map((option) => <option key={option} value={option}>{humanize(option)}</option>)}</select></Field>;
          if (field.kind === 'json') return <StructuredField key={field.key} label={field.label} value={content[field.key]} expected="object-or-array" onChange={(value) => updateContent(index, field.key, value)} />;
          return <Field key={field.key} label={field.label}>{field.kind === 'long' ? <Textarea value={String(content[field.key] ?? '')} onChange={(event) => updateContent(index, field.key, event.target.value)} /> : <Input value={String(content[field.key] ?? '')} onChange={(event) => updateContent(index, field.key, event.target.value)} />}</Field>;
        })}
      </div>}
    </div>;
  })}</div><div style={{ marginBlockStart: '.8rem' }}><Button type="button" onClick={() => onChange([...sections, { sectionType: 'rich_text', isVisible: true, settings: {}, translations: { [locale]: { content: { blocks: [] } } }, _expanded: true }])}>+ Add section</Button></div></div>;
}
...
export function ContentEditor({ resource, id, returnPath }: { resource: string; id?: string; returnPath: string }) {
  ...
  const [record, setRecord] = useState<Record<string, unknown>>({ status: 'draft', translations: {}, sections: [] });
  const [options, setOptions] = useState<Record<string, RelOption[]>>({});
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState('');

  useEffect(() => {
    void api<Language[]>('/admin/languages').then((rows) => { const active = rows.filter((row) => row.isActive); setLanguages(active); setLocale(active.find((row) => row.isDefault)?.code ?? active[0]?.code ?? 'en'); });
    if (id) void api<Record<string, unknown>>(`/admin/${resource}/${id}`).then((data) => setRecord({ ...data, translations: normalizeTranslations(data.translations), sections: normalizeSections(data.sections) }));
  }, [id, resource]);
  useEffect(() => {
    if (!definition) return;
    const resources = new Set(Object.values(definition.relations).map((relation) => relation.resource));
    if (resource === 'pages') Object.values(SECTION_FIELDS).flat().forEach((field) => { if (field.kind === 'relation') resources.add(field.resource); });
    resources.forEach((relationResource) => { void loadOptions(relationResource).then((rows) => setOptions((previous) => ({ ...previous, [relationResource]: rows }))); });
  }, [definition, resource]);
