...
export class MediaService {
  private readonly s3: S3Client; private readonly bucket: string; private readonly secret: string; private readonly publicBaseUrl: string | undefined;
  constructor(private readonly config: ConfigService, private readonly prisma: PrismaService, private readonly queues: QueueService, private readonly audit: AuditService) {
    this.bucket = config.getOrThrow<string>('R2_BUCKET'); this.secret = config.getOrThrow<string>('AUTH_SESSION_SECRET'); this.publicBaseUrl = config.get<string>('R2_PUBLIC_BASE_URL')?.replace(/\/$/, '');
  ...
  private present(row: Record<string, unknown>) {
    ...
    return {
      ...
      url: this.publicBaseUrl && row.storageKey ? `${this.publicBaseUrl}/${String(row.storageKey)}` : undefined,
      variants: variants.map((variant) => ({
        ...variant,
        url: this.publicBaseUrl && variant.storageKey ? `${this.publicBaseUrl}/${String(variant.storageKey)}` : undefined,
      })),
    };
  }
  ...
  async finalize(uploadToken: string, actorId: string, folderId?: string) { const data = this.verify(uploadToken, actorId); const object = await this.s3.send(new HeadObjectCommand({ Bucket: this.bucket, Key: data.storageKey })); if (Number(object.ContentLength) !== data.sizeBytes || object.ContentType !== data.mimeType) throw new BadRequestException('Uploaded object metadata does not match the approved upload.'); const values: Prisma.MediaUncheckedCreateInput = { ...(folderId?{folderId}:{}), storageProvider: 'r2', bucket: this.bucket, storageKey: data.storageKey, originalFilename: data.filename, normalizedFilename: safeName(data.filename), mimeType: data.mimeType, extension: extname(data.filename).slice(1).toLowerCase(), sizeBytes: BigInt(data.sizeBytes), status: data.mimeType.startsWith('image/') ? 'processing' as const : 'ready' as const, uploadedById: actorId }; const replacementId=data.replaceMediaId; const media = replacementId ? await this.prisma.$transaction(async (tx) => { const current = await tx.media.findUniqueOrThrow({ where: { id: replacementId } }); await tx.mediaVariant.deleteMany({ where: { mediaId: current.id } }); const updated = await tx.media.update({ where: { id: current.id }, data: values }); await this.queues.media.add('cleanup-replaced-object', { bucket: current.bucket, storageKey: current.storageKey }); return updated; }) : await this.prisma.media.create({ data: values }); if (data.mimeType.startsWith('image/')) await this.queues.media.add('process', { mediaId: media.id }, { attempts: 3, backoff: { type: 'exponential', delay: 3000 } }); await this.audit.record({ actorUserId: actorId, action: replacementId ? 'media.replaced' : 'media.finalized', entityType: 'media', entityId: media.id }); return media; }
  async list(page = 1, pageSize = 20, q?: string, status?: string, folderId?: string) { const where = { ...(status ? { status: status as 'ready' } : {}), ...(folderId ? { folderId } : {}), ...(q ? { OR: [{ originalFilename: { contains: q, mode: 'insensitive' as const } }, { normalizedFilename: { contains: q, mode: 'insensitive' as const } }] } : {}) }; const [data, total] = await this.prisma.$transaction([this.prisma.media.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { createdAt: 'desc' }, include: { translations: true, variants: true, folder: true } }), this.prisma.media.count({ where })]); return { data: data.map((row) => this.present(row as unknown as Record<string, unknown>)), meta: { page, pageSize, total, pageCount: Math.ceil(total / pageSize) } }; }
  async usages(id: string) {
    const media = await this.prisma.media.findUnique({ where: { id } });
    if (!media) throw new NotFoundException('Media was not found.');

    const [pageTranslations, serviceTranslations, industryTranslations, caseTranslations, insightTranslations, brandTranslations, productTranslations, servicesHero, servicesIcon, serviceCategories, industriesHero, casesHero, caseGallery, insightsCover, insightsDownloadable, teamMembers, clients, partners, brandsLogo, brandsCover, productsLogo, productGallery, testimonials, certifications, sectionTranslations, insightBlocks, settings] = await Promise.all([
      this.prisma.pageTranslation.findMany({ where: { ogMediaId: id }, select: { pageId: true, title: true, locale: true } }),
      this.prisma.serviceTranslation.findMany({ where: { ogMediaId: id }, select: { serviceId: true, title: true, locale: true } }),
      this.prisma.industryTranslation.findMany({ where: { ogMediaId: id }, select: { industryId: true, name: true, locale: true } }),
      this.prisma.caseStudyTranslation.findMany({ where: { ogMediaId: id }, select: { caseStudyId: true, title: true, locale: true } }),
      this.prisma.insightTranslation.findMany({ where: { ogMediaId: id }, select: { insightId: true, title: true, locale: true } }),
      this.prisma.brandTranslation.findMany({ where: { ogMediaId: id }, select: { brandId: true, name: true, locale: true } }),
      this.prisma.productVentureTranslation.findMany({ where: { ogMediaId: id }, select: { productVentureId: true, name: true, locale: true } }),
      ...
      this.prisma.caseStudyMedia.findMany({ where: { mediaId: id }, select: { caseStudyId: true } }),
      ...
      this.prisma.productVentureMedia.findMany({ where: { mediaId: id }, select: { productVentureId: true } }),
      ...
      this.prisma.pageSectionTranslation.findMany({ select: { sectionId: true, locale: true, content: true } }),
      this.prisma.insightTranslation.findMany({ select: { insightId: true, locale: true, content: true } }),
    ...
    return [
      ...
      ...serviceTranslations.map((x) => ({ type: 'service.og', id: x.serviceId, label: `${x.title} (${x.locale})` })),
      ...industryTranslations.map((x) => ({ type: 'industry.og', id: x.industryId, label: `${x.name} (${x.locale})` })),
      ...caseTranslations.map((x) => ({ type: 'case-study.og', id: x.caseStudyId, label: `${x.title} (${x.locale})` })),
      ...insightTranslations.map((x) => ({ type: 'insight.og', id: x.insightId, label: `${x.title} (${x.locale})` })),
      ...brandTranslations.map((x) => ({ type: 'brand.og', id: x.brandId, label: `${x.name} (${x.locale})` })),
      ...productTranslations.map((x) => ({ type: 'product.og', id: x.productVentureId, label: `${x.name} (${x.locale})` })),
      ...
      ...sectionTranslations.filter((x) => references(x.content)).map((x) => ({ type: 'page-section.content', id: x.sectionId, label: x.locale })),
      ...insightBlocks.filter((x) => references(x.content)).map((x) => ({ type: 'insight.content', id: x.insightId, label: x.locale })),
      ...settings.filter((x) => references(x.value)).map((x) => ({ type: 'global-setting', id: x.id, label: x.key })),
    ];
  }
  async retry(id: string, actorId: string) { const media = await this.prisma.media.findUniqueOrThrow({ where: { id } }); if (!media.mimeType.startsWith('image/')) throw new BadRequestException('Only image processing can be retried.'); if (!['failed', 'processing'].includes(media.status)) throw new ConflictException('Only failed or processing media can be retried.'); await this.prisma.media.update({ where: { id }, data: { status: 'processing' } }); await this.queues.media.add('process', { mediaId: id }, { attempts: 3, backoff: { type: 'exponential', delay: 3000 } }); await this.audit.record({ actorUserId: actorId, action: 'media.processing_retried', entityType: 'media', entityId: id }); return { id, status: 'processing' }; }
  ...
  async update(id: string, input: { folderId?: string | null; translations?: Record<string, { title?: string; altText?: string; caption?: string; decorative?: boolean }> }, actorId: string) { const translations = Object.entries(input.translations ?? {}); const clean = (data: { title?: string; altText?: string; caption?: string; decorative?: boolean }) => ({ ...(data.title!==undefined?{title:data.title}:{}), ...(data.altText!==undefined?{altText:data.altText}:{}), ...(data.caption!==undefined?{caption:data.caption}:{}), ...(data.decorative!==undefined?{decorative:data.decorative}:{}) }); const updateData: Prisma.MediaUpdateInput = { ...(input.folderId!==undefined?{folder:input.folderId?{connect:{id:input.folderId}}:{disconnect:true}}:{}), translations: { upsert: translations.map(([locale, data]) => ({ where: { mediaId_locale: { mediaId: id, locale } }, create: { locale, ...clean(data) }, update: clean(data) })) } }; const row = await this.prisma.media.update({ where: { id }, data:updateData, include: { translations: true, variants: true } }); await this.audit.record({ actorUserId: actorId, action: 'media.updated', entityType: 'media', entityId: id }); return row; }
}
