...
const stableHash = (input: unknown) =>
...
export class LeadsService {
  ...
  async submit(
    kind: Extract<LeadSource, 'contact' | 'consultation' | 'assessment'>,
    rawInput: unknown,
    ...
    const countryCode =
      company?.countryCode ?? ('countryCode' in input ? input.countryCode : undefined);
    const message =
      kind === 'assessment' && 'business' in input
        ? input.business.notes
        : 'message' in input
          ? input.message
          : undefined;
    const potentialDuplicate = await this.prisma.lead.findFirst({
      where: {
        OR: [
          { email: contact.email },
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const data: Prisma.LeadUncheckedCreateInput = {
        ...
        ...(kind === 'assessment' && 'business' in input && input.business.industryId
          ? { industryId: input.business.industryId }
          : {}),
        ...(kind === 'consultation' && 'serviceId' in input && input.serviceId
      ...
      await tx.leadActivity.create({
        data: {
          leadId: lead.id,
          type: kind === 'assessment' ? 'assessment_submitted' : 'created',
          payload: { source: kind, duplicateSignaled: Boolean(potentialDuplicate) },
        },
      });
    ...
    await this.queues.email.add(
      'new-lead',
      { leadId: result.submissionId },
      { attempts: 5, backoff: { type: 'exponential', delay: 5000 }, removeOnComplete: 1000 },
    );
    return result;
  }
  ...
  async updateStatus(id: string, status: LeadStatus, actorId: string, requestId: string) {
    const before = await this.prisma.lead.findUniqueOrThrow({
      where: { id },
      select: { status: true },
    });
    ...
    await this.audit.record({
      ...
      entityId: id,
      requestId,
      summary: { from: before.status, to: status },
    });
    return lead;
  }
  async assign(id: string, assignedToUserId: string | null, actorId: string, requestId: string) {
    const lead = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.lead.update({ where: { id }, data: { assignedToUserId } });
      await tx.leadActivity.create({
        data: { leadId: id, actorUserId: actorId, type: 'assigned', payload: { assignedToUserId } },
      });
      return updated;
    });
    await this.audit.record({
  ...
  async note(id: string, body: string, actorId: string, requestId: string) {
    if (!body.trim() || body.length > 5000)
      throw new UnprocessableEntityException('Note must be between 1 and 5000 characters.');
    const note = await this.prisma.$transaction(async (tx) => {
      const created = await tx.leadNote.create({
        data: { leadId: id, authorUserId: actorId, body: body.trim() },
      });
      await tx.leadActivity.create({
        data: {
          leadId: id,
