...
export class WebsiteContentService {
  ...
  async save(resource:string,id:string|undefined,raw:Record<string,unknown>,actorUserId:string,requestId:string) {
    ...
    if(resource==='redirects') {
      const parsed=z.object({sourcePath:pathSchema,destinationPath:pathSchema,statusCode:z.union([z.literal(301),z.literal(302),z.literal(307),z.literal(308)]),locale:z.string().nullable().optional(),active:z.boolean()}).strict().parse(raw);
    ...
    return this.prisma.$transaction(async tx=>{const {items,...parsedValues}=data;const values={key:parsedValues.key,location:parsedValues.location,...(parsedValues.status?{status:parsedValues.status}:{})};const menu=id?await tx.navigationMenu.update({where:{id},data:values}):await tx.navigationMenu.create({data:values});await tx.navigationItem.deleteMany({where:{menuId:menu.id}});
      // Create parents first, retaining editor IDs, then descendants at arbitrary depth.
      const pending=[...items];const inserted=new Set<string>();while(pending.length){const index=pending.findIndex(i=>!i.parentId||inserted.has(i.parentId));const item=pending.splice(index,1)[0]!;await tx.navigationItem.create({data:{id:item.id,menuId:menu.id,parentId:item.parentId,itemType:item.itemType,internalEntityType:item.itemType==='internal'?(item.internalEntityType??null):null,internalEntityId:item.itemType==='internal'?(item.internalEntityId??null):null,externalUrl:item.itemType==='external'?(item.externalUrl??null):null,visible:item.visible,sortOrder:items.indexOf(item),translations:{create:Object.entries(item.translations).map(([locale,t])=>({locale,label:t.label}))}}});inserted.add(item.id);}
      await tx.auditLog.create({data:{actorUserId,action:'navigation.saved',entityType:resource,entityId:menu.id,requestId}});return tx.navigationMenu.findUniqueOrThrow({where:{id:menu.id},include:{items:{include:{translations:true},orderBy:{sortOrder:'asc'}}}});
    });
  }
