...
describe('RBAC seed matrix', () => {
  it('grants every permission to Super Admin', () =>
    expect(new Set(grants.super_admin)).toEqual(new Set(permissions)));
  ...
  it('keeps Viewer read only', () =>
    expect(grants.viewer.every((permission) => permission.endsWith('.read'))).toBe(true));
  it('defines every granted permission in the permission catalog', () => {
    for (const role of roles)
      for (const grant of grants[role]) expect(permissions).toContain(grant);
  });
});
