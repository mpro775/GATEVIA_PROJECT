...
const seo = {
  seoTitle: f('text'),
  seoDescription: f('long'),
  ogTitle: f('text'),
  ogDescription: f('long'),
  ogMediaId: f('media'),
  canonicalUrl: f('url'),
  robotsIndex: f('boolean'),
};
...
export const cmsDefinitions: Record<string, CmsDefinition> = {
  pages: {
    ...
    parent: 'pageId',
    permission: 'pages',
    fields: { featured: f('boolean'), pageType: f('text', true), templateKey: f('text', true) },
    translations: { ...titled, excerpt: f('long'), ...seo },
    relations: { faqs: relation('faqs', 'faqId') },
  },
  'service-categories': {
  ...
  services: {
    ...
    parent: 'serviceId',
    permission: 'services',
    fields: { ...common, heroMediaId: f('media'), iconMediaId: f('media') },
    translations: {
      ...titled,
      shortDescription: f('long', true),
      overview: f('long', true),
      whoFor: f('items'),
      problems: f('items'),
      deliverables: f('items'),
      process: f('items'),
      benefits: f('items'),
      timelineText: f('text'),
      ctaLabel: f('text'),
  ...
  industries: {
    ...
    fields: { ...common, heroMediaId: f('media') },
    translations: {
      ...named,
      shortDescription: f('long', true),
      overview: f('long', true),
      challenges: f('items'),
      opportunities: f('items'),
      ctaLabel: f('text'),
      ...seo,
    },
    relations: {
      services: relation('services', 'serviceId'),
      caseStudies: relation('case-studies', 'caseStudyId'),
      insights: relation('insights', 'insightId'),
    },
  },
  'case-studies': {
    ...
    translations: {
      ...
      clientLabel: f('text'),
      context: f('long', true),
      challenge: f('long', true),
      objectives: f('items'),
      solution: f('long', true),
      process: f('items'),
      results: f('items'),
      metrics: f('items'),
      testimonialText: f('long'),
      ...seo,
    },
    relations: {
      clientId: relation('clients', 'id', false),
      services: relation('services', 'serviceId'),
      industries: relation('industries', 'industryId'),
      gallery: relation('media', 'mediaId'),
    },
  },
  insights: {
    model: 'insight',
    ...
    fields: {
      featured: f('boolean'),
      type: select('article', 'guide', 'report'),
      publishedAt: f('date'),
      coverMediaId: f('media'),
      downloadableMediaId: f('media'),
    },
    translations: { ...titled, excerpt: f('long', true), content: f('blocks', true), ...seo },
    relations: {
      categoryId: relation('insight-categories', 'id', false),
      authorUserId: relation('users', 'id', false),
      tags: relation('tags', 'tagId'),
      services: relation('services', 'serviceId'),
      industries: relation('industries', 'industryId'),
    },
  },
  ...
  faqs: {
    ...
    parent: 'faqId',
    permission: 'faqs',
    fields: { categoryKey: f('text'), sortOrder: f('number') },
    translations: { question: f('text', true), answer: f('long', true) },
    relations: { services: relation('services', 'serviceId'), pages: relation('pages', 'pageId') },
  },
  'team-members': {
    model: 'teamMember',
    parent: 'memberId',
    permission: 'team',
    fields: { photoMediaId: f('media'), linkedinUrl: f('url'), sortOrder: f('number') },
    translations: { name: f('text', true), position: f('text', true), bio: f('long', true) },
    relations: {},
  },
  clients: {
    model: 'client',
    ...
    fields: {
      ...
      website: f('url'),
      countryCode: f('text'),
      publicVisibility: f('boolean'),
    },
    translations: { name: f('text', true), shortDescription: f('long') },
    relations: { industryId: relation('industries', 'id', false) },
  },
  partners: {
    model: 'partner',
    parent: 'partnerId',
    ...
    translations: { name: f('text', true), description: f('long') },
    relations: {},
  },
  brands: {
    model: 'brand',
  ...
  products: {
    ...
    parent: 'productVentureId',
    permission: 'products',
    fields: {
      ...common,
      logoMediaId: f('media'),
      website: f('url'),
      productType: select('product', 'platform', 'venture'),
      launchStatus: select(
        'concept',
        'building',
        'active',
    ...
    relations: {
      industryId: relation('industries', 'id', false),
      gallery: relation('media', 'mediaId'),
    },
  },
  testimonials: {
    model: 'testimonial',
    parent: 'testimonialId',
    permission: 'testimonials',
    fields: {
      ...common,
      personName: f('text', true),
      personRole: f('text'),
      companyName: f('text'),
      countryCode: f('text'),
      logoMediaId: f('media'),
      consentConfirmed: f('boolean'),
    },
    translations: { quote: f('long', true) },
    relations: {
      clientId: relation('clients', 'id', false),
      caseStudyId: relation('case-studies', 'id', false),
    },
  },
  certifications: {
    model: 'certification',
    parent: 'certificationId',
    permission: 'certifications',
    fields: {
      logoMediaId: f('media'),
      issuer: f('text', true),
      certificateNumber: f('text'),
      validFrom: f('date'),
      validUntil: f('date'),
      verificationUrl: f('url'),
      publicVisibility: f('boolean'),
      sortOrder: f('number'),
    },
    translations: { name: f('text', true), description: f('long') },
  ...
  'trust-metrics': {
    model: 'trustMetric',
    parent: 'trustMetricId',
    permission: 'trust_metrics',
    fields: {
      value: f('text', true),
      suffix: f('text'),
      evidenceNoteInternal: f('long'),
      publicVisibility: f('boolean'),
      sortOrder: f('number'),
    },
    translations: { label: f('text', true) },
    relations: {},
  },
};
...
export const safeHref = z
  ...
  .max(2000)
  .refine(
    (value) => /^(https?:\/\/[^\s]+|mailto:[^\s]+|tel:[+\d\s()-]+|\/(?!\/)[^\\\s]*)$/i.test(value),
    'Use a local path or an approved URL protocol.',
  );
export const richBlockSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    headers: z.array(z.string().max(240)).max(12),
...
export const contentItemsSchema = z
  .array(
    z.union([
      z.string().max(4000),
      z
...
export function translationCompleteness(def: CmsDefinition, translation?: Record<string, unknown>) {
  ...
  const missing = required.filter(
    (key) => !translation?.[key] || (Array.isArray(translation[key]) && !translation[key].length),
  );
